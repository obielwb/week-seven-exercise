...
# Exemplo de uso
ClassificarFrete(45)    // "Frete não disponível!"
ClassificarFrete(150)   // "Frete com custo adicional!"
ClassificarFrete(250)   // "Frete grátis!"
```
...
Classe Veiculo:
    ...
    Método CalcularConsumo():
        Retornar "Método genérico, deve ser sobrescrito"
...
Classe Carro herda de Veiculo:
    ...
    Método Construtor(modelo, ano, quilometragem, eficienciaKmPorLitro):
        ...
        Define quilometragem <- quilometragem
        Define eficienciaKmPorLitro <- eficienciaKmPorLitro
...
Classe Moto herda de Veiculo:
    ...
    Método CalcularConsumo():
        consumo <- quilometragem / (eficienciaKmPorLitro * 1.2) // Moto consome menos por eficiência ajustada
        Retornar consumo


carro <- Novo Carro("Fusca", 1970, 1000, 10)
moto <- Novo Moto("CG 125", 2020, 500, 30)
Escrever(carro.CalcularConsumo()) // 100 litros
Escrever(moto.CalcularConsumo())  // ~13.89 litros
```
...
Função CalcularTempoPouso(velocidadeInicial, desaceleracao, velocidadeSegura, tempoMaximo):
    ...
    Enquanto velocidadeAtual > velocidadeSegura faça:
        tempo <- tempo + 1
        velocidadeAtual <- velocidadeInicial - desaceleracao * tempo

        Se tempo > tempoMaximo então:
            Escrever("Tempo máximo excedido! Risco de desvio orbital.")
            Retornar -1
        Fim-se

        Se velocidadeAtual < 0 então:
            Escrever("Desaceleração excessiva! Instabilidade no pouso.")
            Retornar -1
        Fim-se
    Fim-Enquanto
...
# Exemplo de uso
CalcularTempoPouso(1000, 50, 100, 30) // Velocidade inicial: 1000 m/s, desaceleração: 50 m/s², segura: 100 m/s, max: 30s
// Resultado: tempo = 18 segundos
```
...
Função MultiplicarMatrizesInvestimento(matrizA, matrizB):
    ...
    Se colunasA ≠ tamanho(matrizB) então:
...
# Exemplo de uso
investimentosA <- [[1000, 2000], [1500, 2500]]
investimentosB <- [[2, 1], [1, 3]]
resultado <- MultiplicarMatrizesInvestimento(investimentosA, investimentosB)
Escrever("Resultado da multiplicação de investimentos:")
ImprimirMatriz(resultado)
// Resultado: [[4000, 7000], [5500, 9000]]
```
